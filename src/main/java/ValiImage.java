 ...
 *
 * @project_name Day04
 * @class_name ValiImage
 * @author Dovinya
 * @data 2014-8-10 下午03:54:13
 * @version 1
 * @notes 演示输出验证码图片
 */
public class ValiImage extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < 5; i++) {
            g.setColor(new Color(randNum(0, 200), randNum(0, 255), randNum(0,255)));
            g.setFont(new Font("黑体", Font.BOLD, 20));
            int r = randNum(-45, 45);
            g.rotate(1.0*r/180*Math.PI, 5 + i * 30, 25);
            g.drawString(base.charAt(randNum(0, base.length()-1))+"", 5 + i * 30, 25);
            g.rotate(-1.0*r/180*Math.PI, 5 + i * 30, 25);
